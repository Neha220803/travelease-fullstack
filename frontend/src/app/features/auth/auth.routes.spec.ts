...
describe('AUTH_ROUTES', () => {
  ...
  it('defines login, register, and partner-register as children', () => {
    const children = AUTH_ROUTES[0].children ?? [];
    expect(children.map((r) => r.path)).toEqual(['login', 'register', 'partner-register']);
  });

  it('lazily loads the real components for login, register, and partner-register', async () => {
    const children = AUTH_ROUTES[0].children ?? [];
