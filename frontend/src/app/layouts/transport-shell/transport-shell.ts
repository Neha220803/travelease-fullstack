    ...
    <hlm-sidebar-wrapper>
        ...
        <hlm-sidebar-header class="border-b border-sidebar-border">
          <div class="flex items-center gap-2 px-2 py-3">
            <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
              ...
              <ng-icon name="lucideBus" class="text-white" size="16" />
            </div>
            <div class="flex flex-col overflow-hidden group-data-[collapsible=icon]:hidden">
              <span class="text-sm font-bold text-sidebar-foreground">Transport Portal</span>
      ...
      <main hlmSidebarInset>
        <header class="h-14 flex items-center gap-3 px-4 border-b border-border bg-background/95 backdrop-blur sticky top-0 z-20">
          ...
          <span class="text-sm font-medium text-muted-foreground">Transport Provider</span>
...
export class TransportShell {
  private readonly auth = inject(AuthService);
  readonly navItems = [
    { path: '/transport', label: 'Dashboard', icon: 'lucideLayoutDashboard' },
    { path: '/transport/vehicles', label: 'Vehicles', icon: 'lucideBus' },
    { path: '/transport/routes', label: 'Routes', icon: 'lucideRoute' },
    { path: '/transport/bookings', label: 'Bookings', icon: 'lucideCalendarCheck' },
    { path: '/transport/reports', label: 'Reports', icon: 'lucideBarChart2' },
  ];
